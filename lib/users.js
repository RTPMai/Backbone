...
async function saveRoles(roles) {
  ...
  // Force admin to keep full access. An admin who locks themselves out of Settings has
  // no way back in without a redeploy.
  roles.admin = Object.assign({}, roles.admin, {
    protected: true,
    data_scope: "all",
    can_edit: true,
  });
  if (roles.admin.tabs.indexOf("settings") === -1) roles.admin.tabs.push("settings");
